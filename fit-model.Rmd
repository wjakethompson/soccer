...
```{r load_stanfit, include = FALSE, eval = FALSE}
library(rstan)
load("_data/gri_stanfit.rda")
```
...
data_frame(
  ...
  knitr::kable(caption = "Diagnostic statistics for the NUTS algorithm.", align = "c")
```
...
A ranking of the teams can be created by the goals they would be expected to score and concede against an average team at a neutral location. The larger the difference between these expected goals, the better team.
