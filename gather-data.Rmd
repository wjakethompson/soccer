...
To get the URLs for each team, we can write a function that uses the **rvest** package to pull hyperlinks of of the league pages called `scrape_league()`. See Appendix \@ref(league-scrape) for the full function.

```{r league_scrape, include = FALSE}
...
The webpages for these competitions are formatted slightly differently than those for the domestic leagues, so we will need a slightly different function to scrape these team URLs, `scrape_major_cup()` (see Appendix \@ref(cup-scrape)).

```{r uefa_scrape, include = FALSE}
...
These webpages are also formatted differently, so we have one final function to get the team URLs, `scrape_dom_cup()` (see Appendix \@ref(dom-scrape)).

```{r dom_scrape, include = FALSE}
...
Now that we have all of the team URLs, we can scrape the game data from each team's page on ESPN. To do this, we'll define a new scraper function, `scrape_team()` (see Appendix \@ref(game-scrape)). Because there are so many teams to scrape, it's possible we may overload ESPN's server request limit. Therefore, we'll define a safe version of the `read_html()` function that will allow us to check if the website was read correctly.

```{r game_scrape, include = FALSE}
