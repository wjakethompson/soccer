...
```{r ucl-func, eval = FALSE, include = FALSE}
chains <- model_params
team_codes <- team_counts
predict_ucl <- function(matchups = matchups, games = full_data,
  ...
  future_games <- games %>%
    filter(competition == "UCL", date >= ymd(Sys.Date()))
  ...
  mu <- data_list$mu[1]
  eta <- data_list$eta[1]
  alpha <- data_list$alpha[[1]]
  delta <- data_list$delta[[1]]
  sigma_g <- data_list$sigma_g[1]
  sim_games <- future_games
  ...
  ucl_sim <- pmap_df(.l = data_list, .f = function(mu, eta, alpha, delta,
    ...
    }, sim_games = future_games, sim_ucl = matchups, team_codes = team_codes)
}
```
